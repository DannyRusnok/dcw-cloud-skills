...
**v3.5 (2026-07-15, telegram revize — Craftie delivery):** Notes proposal/approval přesunut z Hermése na **Craftie** (substack-mcp `send_telegram_message` s `register:"passive"`, reply-read přes `get_recent_replies`) — 1 bot = 1 téma, Hermés zůstává čistě Threads a bare `ok` v jeho chatu už není nejednoznačné.
...
- Pak pošli **návrhový Telegram preview přes Craftie** (substack-mcp — content-approval bot; telegram revize 2026-07-15: Hermés je od teď čistě Threads): zavolej `send_telegram_message({ text: <celá zpráva>, register: "passive" })`. Z výsledku (`{ok, messageId}`) vezmi `messageId` a **dopiš ho do pending souboru jako `tgMessageId`** — `substack-redraft-watch` podle něj páruje reply-based approval přes `get_recent_replies`. (`register:"passive"` zajistí, že webhook reply na tuhle zprávu jen zaloguje do tg_log a NEpošle do Gaia one-brain.) Fallback (jen když Craftie send selže — chybějící token/síť): Hermés `python %USERPROFILE%\foundary-tools\dcw-context-hub\ops\threads\threads_client.py send-telegram --file <soubor>` a `message_id` → `tgMessageId` jako dřív. Pro každou notu uveď slot + čas + archetyp label, **CELÉ tělo noty — NIKDY ne ořez / prvních N znaků / `…`** (Daniel schvaluje podle úplného textu; useknuté tělo je bug), řádek s grafikou (`🖼 stat_card: <stat>` / `🖼 quote_card: "<quote>"` / `🖼 text-only`) a fact-check řádek. Header nese počet subů z `get_aggregates`. Formát:
  ```
